...
export function generateDockerFiles(app: IApp, outDir: string, db: 'sqlite' | 'postgresql' = 'sqlite', port: number = 4000) {
  // Backend Dockerfile
  ...
  fs.writeFileSync(path.join(outDir, 'backend/Dockerfile'), ejs.render(backendDockerfile, { port }));
  ...
  fs.writeFileSync(path.join(outDir, 'docker-compose.yml'), ejs.render(composeTemplate, { db, port, authProvider: app.config?.auth?.provider }));
}
